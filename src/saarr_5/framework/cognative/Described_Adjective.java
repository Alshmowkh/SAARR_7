 ...
 * @author bakee
 */
public class Described_Adjective extends Entity {

    Sentence sentence;
    Entity entity;

    public Described_Adjective(Sentence sent, Entity ent) {
        sentence = sent;
        this.entity = ent;
    }
    ...
    public Entity getEntity() {
        ...
        Word described = this.subject();
        Word adjective = sentence.get(entity.getBeta().index() + 1);
        if (described != null && adjective != null) {
            this.setAlpha(described);
            this.setBeta(adjective);
            this.setType(Type.ESF_Cg);
            return this;
        }
        return null;
    }
...
//pl(before);
    ...
    public Word getSubject() {
        ...
        Word verb = entity.getAlpha();
        Phrase verbph = sentence.getPhrase(verb.phraseID());
...
//        pl(verbph);
        ...
        if (verbph.index() == 0) {
            candidate = sentence.getPhrase(verbph.index() + 1);
            subj = VS(verb, candidate);
        } else {
            candidate = sentence.getPhrase(verbph.index() - 1);
            subj = SV(candidate, verb);
        }
