...
public class Objector {
    ...
    public Word ObjectOf(Word verb) {
        if (!sentence.hasVerb()) {
            return null;
        }
        Phrase verbph = null;
...
//            System.out.println(verb + " is passive verb.");
        ...
        if (verb.per() != null && verb.per().equals("2")) {
            ...
            return this.implicitObject(verb);
        }
//        pl(verbph);
    ...
    public Word object() {
        ...
        if (verb.hasEnc() && verb.enc0().endsWith("obj")) {
            return this.implicitObject(verb);
        }
        ...
        if (verbph.index() == 0) {
            candidate = sentence.getPhrase(verbph.index() + 1);
            if(subject.isLetter){
                object = VSO(verb, candidate);
            }
//            if (this.pronouns().containsValue(subject.value())) {
//                object = VSO(verb, candidate);
//            }
        } else {
            candidate = sentence.getPhrase(verbph.index() - 1);
            object = SV(candidate, verb);
        }

        return object;
    }

    private Word VSO(Word verb, Phrase get) {
        if (get == null) {
            return null;
        }
        String per = verb.per();
...
//        pl(verb.enc0());
        if ("1".equals(per)) {
            ...
            if (get.isPRP()) {
                ...
                subject = implicitObject(verb);
            }

        } else if ("3".equals(per)) {
            subject = Objector.this.guessObj(verb, get);
        }
        return subject;
    }
...
//pl(before);
        ...
        subject = this.guessObj(before, verb);

        if (subject == null && sentence.hasPhrase(i + 2)) {
            resubject = this.VSO(verb, sentence.getPhrases().get(i + 2));
            if (resubject != null) {
                return resubject;
            }
        }
    ...
    private Word implicitObject(Word verb) {
        Word prn = new Word();
        prn.tag("im_pron");
        String pron = verb.enc0().split("_")[0];
    ...
    private Word guessObj(Word verb, Phrase phr) {
        Word noun = null;
        ...
        if (i < sentence.getPhrases().size() - 1) {
            ...
            return Objector.this.guessObj(verb, phr);
        }
    ...
    private Word guessObj(Phrase phr, Word verb) {
